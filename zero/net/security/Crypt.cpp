...
#include <zero/net/security/Checksum.h>
...
void ContinuumEncrypt::FinalizeExpansion(u32 key) {
  ...
  // Perform final step of key expansion by running it through a modified MD5
  // hasher
  expanded_key[0] ^= key1;
...
size_t ContinuumEncrypt::Encrypt(const u8* pkt, u8* dest, size_t size) {
  ...
  // Perform crc escape if the crc ends up being 0xFF or encrypted packet looks
  // like connection init packet
