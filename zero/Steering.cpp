...
inline static float GetMaxSpeed(Game& game) {
  // We increase the max speed to handle inaccuracies of 40 rotations.
  // Without this increase, slight changes of the same subspace-rotation would create wiggling rotations.
  constexpr float kMaxSpeedMultiplier = 1.1f;
  ...
  float speed = int_speed / 16.0f / 10.0f;

  return speed * kMaxSpeedMultiplier;
}

void Steering::Face(Game& game, const Vector2f& target) {
  Player* self = game.player_manager.GetSelf();
...
void Steering::SeekZero(Game& game) {
  Player* self = game.player_manager.GetSelf();
...
void Steering::Seek(Game& game, const Vector2f& target) {
  Player* self = game.player_manager.GetSelf();
...
void Steering::Seek(Game& game, const Vector2f& target, float target_distance) {
  Player* self = game.player_manager.GetSelf();
...
void Steering::Arrive(Game& game, const Vector2f& target, float slow_radius, float min_multiplier) {
  Player* self = game.player_manager.GetSelf();
...
void Steering::Pursue(Game& game, const Vector2f& target_position, const Player& target, float target_distance) {
  Player* self = game.player_manager.GetSelf();
  float max_speed = GetMaxSpeed(game);
...
void Steering::AvoidWalls(Game& game) {
...
void Steering::AvoidTeam(Game& game, float dist) {
...
void Steering::AvoidEnemy(Game& game, float dist) {
  auto& pm = game.player_manager;
