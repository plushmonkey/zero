...
bool BackgroundRenderer::Initialize(MemoryArena& perm_arena, MemoryArena& temp_arena, const Vector2f& surface_dim) {
  static_assert(ZERO_ARRAY_SIZE(textures) % 2 == 0, "Background texture array size must be even.");
  glGenTextures(ZERO_ARRAY_SIZE(textures), textures);
  ...
  for (size_t i = 0; i < count_total; ++i) {
    size_t texture_index = rand() % (ZERO_ARRAY_SIZE(textures) / 2);
  ...
  for (size_t i = count_total; i < count_total * 2; ++i) {
    ...
    renderables[i].texture = textures[texture_index + ZERO_ARRAY_SIZE(textures) / 2];
    renderables[i].dimensions = Vector2f(kTextureDim, kTextureDim);
...
void BackgroundRenderer::Cleanup() {
  glDeleteTextures(ZERO_ARRAY_SIZE(textures), textures);
}
