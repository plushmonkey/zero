...
static std::unique_ptr<behavior::BehaviorNode> CreateAimTree(behavior::ExecuteContext& ctx) {
  ...
  constexpr float kAdvantageLeashDistance = 15.0f;
  ...
  builder
    .Sequence()
        ...
        .Sequence(CompositeDecorator::Success) // Determine main movement vectors
            ...
            .Selector()
#if 0
                .Sequence() // If we have more people than the enemy, get closer to them.
                    .Child<TurretCountQueryNode>("nearest_target", "enemy_turret_size")
                    .Child<TurretCountQueryNode>("self_turret_size")
                    .Child<ScalarThresholdNode<size_t>>("self_turret_size", "enemy_turret_size")
                    .Child<SeekNode>("nearest_target_position", kAdvantageLeashDistance, SeekNode::DistanceResolveType::Dynamic)
                    .End()
#endif
